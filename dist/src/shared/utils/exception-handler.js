...
export const ExceptionHandler = (err) => {
    const errors = {
        P2002: () => new UniqueConstraintException(`${err.meta?.target} is already in use`),
        P2025: () => new BadRequestException(`Record not found`),
        P2000: () => new BadRequestException(`field exceeds the maximum length`),
        PrismaClientValidationError: () => new BadRequestException(`The field is missing or invalid`),
    };
